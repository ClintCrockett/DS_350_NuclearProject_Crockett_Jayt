...
heights_raw <- read_excel("data/Height.xlsx", skip = 1)  # adjust skip as needed
...
heights <- heights_raw %>%
  ...
  mutate(
    height_cm = as.numeric(height_cm),       # make sure values are numeric
    year_decade = as.numeric(year_decade),
    height_in = height_cm / 2.54
  ) %>%
  filter(!is.na(height_cm))                  # remove rows with missing height
